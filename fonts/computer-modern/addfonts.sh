...
#
#   Licengsed under the Apache License, Version 2.0 (the "License"); you may not
...
availableTTF=( $(ls -1 *.[oOtT][tT][fF] | gsed 's/\.otf\|\.ttf//i') )
installedPFB=( $(grep -e '^/' Fontmap | gsed 's/^.*(\|\.pfb.*$//g') )
...
for ff in ${queueFonts[@]}; do
    ...
    familyNames=("${familyNames[@]}" \
        "$(grep FamilyName $ff.afm | cut -d' ' -f2- | gsed 's/ //g')")
    faceNames=("${faceNames[@]}" \
        "$(grep Weight $ff.afm | cut -d' ' -f2- | \
        gsed -e' s/^./\U&/g; s/ *//g; s/Regular/Base/i; s/Italic/Slope/i ')")
    tagNames=("${tagNames[@]}" "${familyNames[@]:(-1)}-${faceNames[@]:(-1)}")

    echo "/${fontNames[@]:(-1)}						 ($ff.pfb);" >>Fontmap
    
    if [[ -z `cat fontdefs.ld | gsed -n '/'"$ff"'.afm/p'` ]] ; then
        cat >>fontdefs.ld<<LOUT_ENTRY 
{ @FontDef
      @Tag { ${tagNames[@]:(-1)} }
